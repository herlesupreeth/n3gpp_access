...
Script to retrieve and extract asn1 description from 3gpp specs
...
Example for NGAP ASN1
$ rm asn1/*
$ ./asn1_fetch.py -v 16.6.0 -o asn1 -ts 38413 -p NGAP_

Example for NR-RRC ASN1
$ ./asn1_fetch.py -v 16.5.0 -o asn1 -ts 38331 -p NRRRC_
...
class Ts3gpp(object):
    ...
    def unzip_and_convert_to_text(zipdata):
        ...
        for name in zip_file.namelist():
            if name[-4:] == ".doc":
                ...
                suffix = ".docx"
        if docname is None:
            raise RuntimeError("No .doc/.docx found")
        print "Found document : " + docname
        data = zip_file.read(docname)
        with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp_doc_file:
    ...
    @staticmethod
    def text_to_asn1(output_dir, text, prefix):
    ...
    def convert_ts_to_asn1(self, version, output_dir, prefix):
        zipcontent = self.get_zip_archive(version)
        ts_as_text = Ts3gpp.unzip_and_convert_to_text(zipcontent)
        Ts3gpp.text_to_asn1(output_dir, ts_as_text, prefix)
...
def main():
    ...
    parser.add_argument('-o', '--output-directory', type=dir_path,
                        help='output directory for asn.1 files', default="./asn1")
    parser.add_argument('-ts', help='TS reference ', type=int)
    parser.add_argument('-v', '--version', help='TS version')
    parser.add_argument('-p', '--prefix', help='Prefix for file names')
    ...
    ts3gpp.convert_ts_to_asn1(version, args.output_directory, prefix)
