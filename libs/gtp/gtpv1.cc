...
std::unique_ptr<Header> Header::Decode(const OctetBuffer &pdu,
  ...
  if (((flags >> 4) & 0x01) != 1) {
	GTPV1_LOG_ERR("Invalid Protocol Type for GTP")
	return nullptr;
  }
  ...
  // 8 Bytes mandatory header of GTPV1
  idx += 8;
...
std::unique_ptr<UdpPortExtHdr> UdpPortExtHdr::Decode(const OctetBuffer &pdu,
  ...
  if (pdu.GetUint8(idx) != static_cast<uint8_t>(ExtHdrType::udpPort)) {
	GTPV1_LOG_ERR("Unsupported Extension Header Type.")
	return nullptr;
  }
  ...
  // Check for length
  if (length != 0x01) {
	GTPV1_LOG_ERR("Invalid length for UDP Port Extension Header.")
	return nullptr;
  }
...
std::unique_ptr<PdcpPduNumberExtHdr> PdcpPduNumberExtHdr::Decode(
  ...
  // Check for length
  if (length != 0x01) {
	GTPV1_LOG_ERR("Invalid length for PDCP PDU Number Extension Header.")
	return nullptr;
  }
  // 15 bits only
...
std::unique_ptr<LongPdcpPduNumberExtHdr> LongPdcpPduNumberExtHdr::Decode(
  ...
  // Check for length
  if (length != 0x02) {
	GTPV1_LOG_ERR("Invalid length for Long PDCP PDU Number Extension Header.")
	return nullptr;
  }
  // 18 bits only
...
std::unique_ptr<ServiceClassIndicatorExtHdr> ServiceClassIndicatorExtHdr::Decode(
  ...
  // Check for length
  if (length != 0x01) {
	GTPV1_LOG_ERR("Invalid length for Service Class Indicator Extension Header.")
	return nullptr;
  }
...
std::unique_ptr<RecoveryIe> RecoveryIe::Decode(const OctetBuffer &pdu,
  ...
  if (pdu.GetUint8(idx) != static_cast<uint8_t>(IeType::recovery)) {
	GTPV1_LOG_ERR("Unsupported Ie Type.")
	return nullptr;
  }
